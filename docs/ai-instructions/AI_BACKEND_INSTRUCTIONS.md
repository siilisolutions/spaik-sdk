...
### BaseAgent (`spaik_sdk/agent/base_agent.py:32`)
...
- **`get_response_stream(user_input: Optional[str]) -> AsyncGenerator[Dict[str, Any], None]`** (`spaik_sdk/agent/base_agent.py:62`)
...
- **`get_event_stream(user_input: Optional[str]) -> AsyncGenerator[ThreadEvent, None]`** (`spaik_sdk/agent/base_agent.py:67`)
...
- **`get_response(user_input: Optional[str]) -> ThreadMessage`** (`spaik_sdk/agent/base_agent.py:75`)
...
- **`get_response_async(user_input: Optional[str]) -> ThreadMessage`** (`spaik_sdk/agent/base_agent.py:78`)
  - Async version of get_response
  - Non-blocking, returns awaitable ThreadMessage
  - Preferred for async applications

- **`get_response_text(user_input: Optional[str]) -> str`** (`spaik_sdk/agent/base_agent.py:87`)
  - Convenience method returning just text content
  - Synchronous, blocks until completion
  - Use for simple text-only responses

- **`get_response_text_async(user_input: Optional[str]) -> str`** (`spaik_sdk/agent/base_agent.py:90`)
  - Async version of get_response_text
  - Returns awaitable string
  - Preferred for async text-only responses

- **`get_structured_response(prompt: str, output_schema: Type[T]) -> T`** (`spaik_sdk/agent/base_agent.py:93`)
...
- **`set_thread_container(thread_container: ThreadContainer) -> None`** (`spaik_sdk/agent/base_agent.py:123`)
  - Sets custom thread container for conversation state
  - Automatically subscribes to thread events

- **`set_cancellation_handle(cancellation_handle: Optional[CancellationHandle]) -> None`** (`spaik_sdk/agent/base_agent.py:129`)
  - Sets cancellation handle for request cancellation
  - Use for implementing request timeouts and cancellation

**CLI Method:**

- **`run_cli() -> None`** (`spaik_sdk/agent/base_agent.py:101`)
...
#### ToolProvider (`spaik_sdk/tools/tool_provider.py`)
...
#### ModelRegistry (`spaik_sdk/models/model_registry.py`)
...
#### LLMConfig (`spaik_sdk/models/llm_config.py`)
...
#### ThreadContainer (`spaik_sdk/thread/thread_container.py`)
...
#### ThreadMessage (`spaik_sdk/thread/models.py`)
...
#### ConditionalRecorder (`spaik_sdk/recording/conditional_recorder.py`)
...
#### env_config (`spaik_sdk/config/env.py`)
